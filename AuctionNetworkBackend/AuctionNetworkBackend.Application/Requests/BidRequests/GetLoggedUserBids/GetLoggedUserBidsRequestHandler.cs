...
{
    public class GetLoggedUserBidsRequestHandler : IRequestHandler<GetLoggedUserBidsRequest, PagedResult<GetLoggedUserBidsDto>>
    {
        ...
        public GetLoggedUserBidsRequestHandler(IListingRepository listingRepository, IUserContextService userContextService, IUserRepository userRepository, ICategoryRepository categoryRepository)
        {
        ...
        public async Task<PagedResult<GetLoggedUserBidsDto>> Handle(GetLoggedUserBidsRequest request, CancellationToken cancellationToken)
        {
            ...
            var listings = await _listingRepository.GetLoggedUserBids(loggedUserId);

            var pageSize = 6;

            var listingsDto = listings.Select(x => new GetLoggedUserBidsDto
            {
                SellerId = x.SellerId,
            ...
            var pagedResult = new PagedResult<GetLoggedUserBidsDto>(listingsDto, listingsDto.Count, pageSize, request.PageNumber);
