...
{
    public class GetLoggedUserSalesRequestHandler : IRequestHandler<GetLoggedUserSalesRequest, PagedResult<GetLoggedUserSalesDto>>
    {
        private readonly IListingRepository _listingRepository;
        private readonly IUserContextService _userContextService;
        private readonly IUserRepository _userRepository;
        private readonly ICategoryRepository _categoryRepository;
        public GetLoggedUserSalesRequestHandler(IListingRepository listingRepository, IUserContextService userContextService, IUserRepository userRepository, ICategoryRepository categoryRepository)
        {
        ...
        public async Task<PagedResult<GetLoggedUserSalesDto>> Handle(GetLoggedUserSalesRequest request, CancellationToken cancellationToken)
        {
            ...
            var listings = await _listingRepository.GetListingsBySellerId(loggedUserId);

            var pageSize = 6;

            var listingsDto = listings.Select(x => new GetLoggedUserSalesDto
            {
                SellerId = x.SellerId,
            ...
            var pagedResult = new PagedResult<GetLoggedUserSalesDto>(listingsDto, listingsDto.Count, pageSize, request.PageNumber);
